...
public interface BloodDonorRepository extends JpaRepository<BloodDonor, Long> {
    ...
    @Query(value="SELECT * FROM donors WHERE blood_group = ?3 AND ST_DWithin(location::geography, CAST(?1 AS geography), radius + ?2)", nativeQuery = true)
    Iterable<BloodDonor> findMatchingDonors(Point otherLocation, double otherRadius, BloodGroup bloodGroup);
